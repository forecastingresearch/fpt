export default class Shipley_Vocab {
    ...
    get_default_settings() {
        ...
        settings.DATA = [
            {id: "shipley_vocab_0", prompt: "<b>0</b>. LARGE", options: ["red", "big", "silent", "wet"], correct: "big"},    
        ]
    ...
    define_trials() {
        ...
        self.general_instructions = {
            ...
            pages: [`<p class="instructions-title" style="text-align: center">Vocabulary test</p>
                    <p>In this task, you will see a word in CAPITAL LETTERS and four words below it.</p>
                    <p>Select the word that has the same meaning as the one in CAPITAL LETTERS.</p>
                    <p>Example:</p>
                    <div style="width: 90%; margin: auto;"><img style="width: 100%; border: 2px solid black;" src="img/instructions/shipley_vocab_example.png"/></div>
            ...
            on_finish: function(data) {
                data.trial_name = "shipley_vocab_instructions"
            }
        }
        ...
        self.test_trial = {
            ...
            on_finish: function(data) {
                data.trial_name = "shipley_vocab_test_trial"
                
                let score = 0
                if (data.response !== undefined) {
                    for (const [shipley_id, answer] of Object.entries(data.response)) {
                        if (answer === self.settings.DATA.filter(e=>e.id===shipley_id)[0].correct) {
                            score++
                        }
                    }
                }
                data.shipley_vocabulary_score = score
            }
        }
    }
    ...
    get_timeline() {
        ...
        if (this.Experiment.saved_progress.data_checkpoints.includes(`${this.prettyname}__completed`)) {
            ...
            timeline.push(this.Experiment.trial_generators.add_task_stylesheet("shipley_vocab_task"))
            if (this.Experiment.settings.INSTRUCTIONS_ONLY) {
                timeline.push({type: jsPsychInstructions, pages: ['SIGNPOST: Shipley (vocab) - instructions'], show_clickable_nav: true})
            ...
            timeline.push(this.Experiment.trial_generators.remove_task_stylesheet("shipley_vocab_task"))
