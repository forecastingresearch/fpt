export default class ADMC_Framing_RC1 {
    constructor(jsPsych, Experiment, task_query_string) {
        this.duration_mins = 3
        this.prettyname = "ADMC Framing RC1"
    ...
    get_timeline() {
        ...
        if (this.Experiment.saved_progress.data_checkpoints.includes(`${this.prettyname}__completed`)) {
            ...
            if (this.Experiment.settings.INSTRUCTIONS_ONLY) {
                timeline.push({type: jsPsychInstructions, pages: ['SIGNPOST: ADMC RC1 instructions'], show_clickable_nav: true})
                timeline.push(this.rc1_instructions)
            } else {
                timeline.push(this.rc1_instructions)
                timeline.push(this.Experiment.hide_progress_bar())
                timeline.push({timeline: [this.resistance_to_framing_trial], timeline_variables: this.settings.RESISTANCE_TO_FRAMING_ITEMS.filter(e=>e.id.startsWith('rc1_'))})
            }
            timeline.push(this.Experiment.update_progress_bar(this.duration_mins))
