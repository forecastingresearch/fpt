    ...
    get_timeline() {
        ...
        if (this.Experiment.saved_progress.data_checkpoints.includes(`${this.prettyname}__completed`)) {
            ...
            if (this.Experiment.settings.INSTRUCTIONS_ONLY) {
                timeline.push({type: jsPsychInstructions, pages: ['SIGNPOST: ADMC A1 instructions'], show_clickable_nav: true})
                timeline.push(this.a1_instructions)
            } else {
                timeline.push(this.a1_instructions)
                timeline.push(this.Experiment.hide_progress_bar())
                timeline.push({timeline: [this.resistance_to_framing_trial], timeline_variables: this.settings.RESISTANCE_TO_FRAMING_ITEMS.filter(e=>e.id.startsWith('a1_'))})
            }
            timeline.push(this.Experiment.update_progress_bar(this.duration_mins))
